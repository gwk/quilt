...
def gen_vec(orig_type, dim, s_type, fs_type, v_type, v_prev, is_simd, is_novel):
  ...
  outL('  public typealias Scalar = $', s_type)
  outL('  public typealias FloatType = $', fs_type)
  ...
  if is_novel:
    for c in comps:
      outL('  public var $: Scalar', c)
    outL('  public init($) {', jcf('_ $: Scalar', comps))
    for c in comps:
      outL('    self.$ = $', c, c)
    outL('  }')
  ...
  outL('  public var sqrLen: FloatType { return ($) }', jf(' + ', 'FloatType($).sqr', comps))
  outL('  public var aspect: FloatType { return FloatType(x) / FloatType(y) }')
  ...
  if is_float:
    outL('')
    outL('  public var allNormal: Bool { return $ }', jf(' && ', '$.isNormal', comps))
    outL('  public var allFinite: Bool { return $ }', jf(' && ', '$.isFinite', comps))
    outL('  public var allZero: Bool { return $ }', jf(' && ', '$.isNormal', comps))
    outL('  public var anySubnormal: Bool { return $}', jf(' || ', '$.isSubnormal', comps))
    outL('  public var anyInfite: Bool { return $}', jf(' || ', '$.isInfinite', comps))
    outL('  public var anyNaN: Bool { return $}', jf(' || ', '$.isNaN', comps))
    outL('  public var norm: $ { return self / self.len }', v_type)
    outL('  public var clampToUnit: $ { return $($) }', v_type, v_type, jcf('clamp($, min: 0, max: 1)', comps))
    outL('  public var toU8Pixel: VU8Type { return VU8Type($) }', jcf('U8(clamp($ * 255, min: 0, max: 255))', comps))
    outL('  public var heading: Scalar { return atan2(y, x) }')
    outL('')
    outL('  public func dot(_ b: $) -> Scalar { return $ }',
      v_type, ' + '.join(fmt('($ * b.$)', c, c) for c in comps))
    outL('  public func angle(_ b: $) -> Scalar { return acos(self.dot(b) / (self.len * b.len)) }',
      v_type)
    outL('  public func lerp(_ b: $, _ t: Scalar) -> $ { return self * (1 - t) + b * t }',
      v_type, v_type)
  ...
  for op in ops:
    ...
    outL('public func $(a: $, b: $) -> $ { return $($) }', op, v_type, v_type, v_type, v_type, cons_comps_v)
  for op in ops:
    cons_comps_s = jc(fmt('$ $ s', a, op) for a in comps_a) # e.g. 'a.x + s'.
    outL('public func $(a: $, s: $) -> $ { return $($) }', op, v_type, s_type, v_type, v_type, cons_comps_s)

  if is_signed:
    outL('public prefix func -(a: $) -> $ { return a * -1 }', v_type, v_type)

...
def main():
  if len(argv) > 1:
    ...
    parser.add_argument('orig_type')
    parser.add_argument('dimension', type=int)
    parser.add_argument('scalar_type')
    parser.add_argument('float_scalar_type')
    parser.add_argument('import_names', nargs='+')
...
import simd''')
  ...
  if args:
    orig_type = args.orig_type
    if orig_type[-1] in ('3', '4'):
      orig_type_dim = int(orig_type[-1])
      v_prev = f'{orig_type[:-1]}{orig_type_dim-1}'
    else:
      v_prev = None
    for import_name in args.import_names:
      outL('import $', import_name)
    gen_vec(orig_type, args.dimension, args.scalar_type, args.float_scalar_type,
      v_type=orig_type, v_prev=v_prev, is_simd=False, is_novel=False)
